  ...
  async handleApi(req, res, parsedUrl) {
    ...
    if (req.method === 'GET' && route === '/api/team/invites') {
      try {
        ...
        const rows = onlyOpen
          ...
          : invites;
        sendJson(res, 200, { ok: true, workspace, invites: rows });
      } catch (error) {
        sendJson(res, 400, { ok: false, error: String(error?.message || error || '') });
      }
      return;
    ...
    if (req.method === 'POST' && route === '/api/team/invites') {
      try {
        ...
        const invite = opsStorage.createInvite({ workspace, role, actor, expiresInHours });
        const invites = opsStorage.listInvites({ workspace, includeExpired: true });
        sendJson(res, 200, { ok: true, workspace, invite, invites });
      } catch (error) {
        sendJson(res, 400, { ok: false, error: String(error?.message || error || '') });
      }
      return;
