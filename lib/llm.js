...
function getAgentConfig() {
  ...
  return {
    provider: cfg.provider || process.env.META_AGENT_PROVIDER || 'openai',
    model: cfg.model || process.env.META_AGENT_MODEL || '',
    apiKey: cfg.apiKey || process.env.META_AGENT_API_KEY || process.env.OPENAI_API_KEY || ''
  };
}
...
async function maybePromptForApiKey(provider) {
  ...
  if (!process.stdout.isTTY) {
    throw new Error('Missing LLM API key (set META_AGENT_API_KEY or configure via config).');
  }

  console.log(chalk.yellow('\nLLM planning needs an API key.'));
  console.log(chalk.gray('You can set it via env var META_AGENT_API_KEY, or enter it now.\n'));
  ...
  const ans = await inquirer.prompt([
    ...
    },
    {
      ...
      name: 'save',
      default: false,
      message: 'Save this key in ~/.meta-cli/config.json? (not recommended for shared machines)'
    }
  ]);
...
async function planWithLLM({ provider, model, intent, scope, tools, memorySummary }) {
  ...
  throw new Error(`Provider not implemented yet: ${p}. Use --provider openai or set META_AGENT_PROVIDER=openai.`);
}
