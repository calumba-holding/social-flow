...
class AutonomousAgent {
  ...
  async heuristicDecision(userInput) {
    ...
    if (!intent || !intent.action || intent.action === 'unknown_input' || !this.isSupportedTool(intent.action)) {
      return {
        message: 'I am not fully sure what you want yet. Tell me the exact goal, like: "do I have a Facebook page", "check auth status", or "post this to Facebook".',
        actions: [],
        needsInput: true,
        suggestions: [
          'Try: "do I have a Facebook page?"',
          'Try: "check my rate limits"',
