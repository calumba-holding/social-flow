...
class MetaApiClient {
  constructor(opts) {
    ...
    this.token = token || '';
    this.apiVersion = apiVersion || config.getApiVersion();
  ...
  handleError(error, hints = {}) {
    if (error?.response?.data?.error) {
      ...
      if (code === 190) {
        console.error(chalk.cyan('\n  Hint: Token expired/invalid. Re-run: meta auth login'));
        ...
        if (hints.scopes && hints.scopes.length) {
          ...
          console.error(chalk.cyan('  Re-auth with: meta auth login --scopes'));
        }
