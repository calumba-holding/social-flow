...
const { planWithLLM, planWithHeuristics } = require('./llm');
...
async function runAgent({ intent, options }) {
  ...
  try {
    ...
    if (spinner) spinner.stop();
    if (!options.json) {
      console.log(chalk.yellow('! LLM planning unavailable or failed; using safe heuristic plan.'));
      console.log(chalk.gray(`  Reason: ${e?.message || String(e)}`));
    }
    plan = planWithHeuristics({ intent, scope, tools, memorySummary: summary });
  }
  ...
  while (true) {
    ...
    if (choice === 'edit') {
      ...
      try {
        ...
        if (reSpinner) reSpinner.stop();
        plan = planWithHeuristics({ intent: currentIntent, scope, tools, memorySummary: summary });
      }
      continue;
    }
