...
async function aiParseIntent(text, cfg = {}) {
  ...
  if (!parsed) {
    logDebug(debug, 'Deterministic parser returned empty; falling back to heuristic parser.');
    return heuristicParse(input);
  }
  const normalized = normalizeIntent(parsed);
  if (normalized.action === 'unknown_input') {
    logDebug(debug, 'Deterministic parser returned unknown_input; falling back to heuristic parser.');
    return heuristicParse(input);
  }
