...
exports.inrAmountFromText = inrAmountFromText;
exports.parseBhk = parseBhk;
exports.parseCity = parseCity;
exports.parseLocality = parseLocality;
exports.parsePossession = parsePossession;
exports.parseWhatsAppNumber = parseWhatsAppNumber;
exports.isKnownCity = isKnownCity;
exports.parseBriefText = parseBriefText;
exports.formatInr = formatInr;
exports.requiredFields = requiredFields;
exports.briefComplete = briefComplete;
exports.formatBrief = formatBrief;
exports.mergeBrief = mergeBrief;
exports.briefFromOptions = briefFromOptions;
const INDIA_CITIES = [
    "mumbai",
    "delhi",
    "ncr",
...
const CITY_PATTERN = new RegExp(`\\b(${INDIA_CITIES.map((c) => c.replace(/\s+/g, "\\s+")).join("|")})\\b`, "i");
function normalizeText(value) {
    return String(value ?? "")
        .trim()
        .replace(/\s+/g, " ");
}
function inrAmountFromText(text) {
...
function parseLocality(text) {
    ...
    for (const pattern of patterns) {
        ...
        if (city && candidate.toLowerCase() === city.toLowerCase())
            continue;
        if (INDIA_CITIES.includes(candidate.toLowerCase()))
            continue;
        const words = candidate.split(/\s+/);
        if (words.some((word) => INDIA_CITIES.includes(word.toLowerCase())))
            continue;
...
function parsePossession(text) {
    ...
    const dateMatch = raw.match(/\bpossession\s+(?:in|by)\s+([a-z]{3,9}\s+\d{4}|\d{4}|[a-z]{3,9})\b|(?:possession|by)\s+([a-z]{3,9}\s+\d{4})\b/i);
...
function parseWhatsAppNumber(text) {
    const raw = normalizeText(text);
...
function isKnownCity(value) {
    return INDIA_CITIES.includes(String(value || "").trim().toLowerCase());
}
function parseBriefText(text) {
    ...
    if (propertyTypeMatch) {
        ...
        if (value === "flat")
            brief.propertyType = "apartment";
        else if (value === "penthouse")
            brief.propertyType = "apartment";
        else if (value === "row house")
            brief.propertyType = "independent_house";
        else
            brief.propertyType = value;
    }
    ...
    if (possession.possession)
        brief.possession = possession.possession;
    if (possession.possessionDate)
        brief.possessionDate = possession.possessionDate;
...
function requiredFields(brief, includeIds = true) {
    const missing = [];
    if (!brief.city && !brief.locality)
        missing.push("city");
    if (!brief.dailyBudget)
        missing.push("dailyBudget");
    if (!brief.whatsappNumber)
    ...
    if (includeIds) {
        ...
        if (!brief.adAccountId)
            missing.push("adAccountId");
    }
    return missing;
}
function briefComplete(brief, includeIds = true) {
...
function formatBrief(brief) {
    ...
    if (brief.dailyBudget)
        lines.push(`Daily ad budget: ${formatInr(brief.dailyBudget)}`);
    if (brief.whatsappNumber)
        lines.push(`WhatsApp: ${brief.whatsappNumber}`);
    if (brief.message)
        lines.push(`Ad message: ${brief.message}`);
    if (brief.pageId)
        lines.push(`Facebook page ID: ${brief.pageId}`);
    if (brief.adAccountId)
        lines.push(`Ad account: ${brief.adAccountId}`);
    if (brief.image)
        lines.push(`Image: ${brief.image}`);
    return lines.join("\n");
}
function mergeBrief(base, incoming) {
    return { ...base, ...incoming };
}
function briefFromOptions(opts) {
    const brief = {};
    if (opts.projectName)
        brief.projectName = String(opts.projectName);
    if (opts.builderName)
        brief.builderName = String(opts.builderName);
    if (opts.propertyType)
        brief.propertyType = String(opts.propertyType);
    if (opts.bhk)
        brief.bhk = String(opts.bhk);
    if (opts.city)
        brief.city = String(opts.city);
    if (opts.locality)
        brief.locality = String(opts.locality);
    if (opts.price)
        brief.price = String(opts.price);
    if (opts.possession)
        brief.possession = String(opts.possession);
    if (opts.possessionDate)
        brief.possessionDate = String(opts.possessionDate);
    ...
    if (opts.pageId)
        brief.pageId = String(opts.pageId);
    if (opts.adAccountId)
        brief.adAccountId = String(opts.adAccountId);
    if (opts.whatsappNumber)
        brief.whatsappNumber = String(opts.whatsappNumber);
    if (opts.image)
        brief.image = String(opts.image);
    if (opts.message)
        brief.message = String(opts.message);
    if (opts.leadFormId)
        brief.leadFormId = String(opts.leadFormId);
    if (opts.destination)
        brief.destination = opts.destination === "lead_form" ? "lead_form" : "whatsapp";
    return brief;
}
